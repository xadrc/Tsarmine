...
resp_unit	<- "deg/w"
resp_meas 	<- "weekly cumulative"
resp_range	<- c(-.2, 5.2)

var	 		<- df_evo$wtemp
var_name	<- "temperatures"
var_unit 	<- "°C"
var_meas	<- "weekly means"
var_range 	<- c(-10, 22)

d_			<- data.frame(resp, var)
...
model_name		<- "Temperatures - weekly means"
...
reglin <- data.frame(
    x = seq(
        extendrange(d_keep$var, f = .05)[1],
        extendrange(d_keep$var, f = .05)[2],
        .01
    ),
    y = NA
)
...
quartz.save(
    file = file.path(dir_plt, "models", "mdl_23.png"),
    type = "png",
    dpi = 300
)
