...
var	 			<- df_gps$speed_week
var_name		<- "creep"
var_unit 		<- "m/a"
var_meas		<- "weekly displacement"
var_range 		<- c(3.5, 8.5)
...
l_mod		<- lm(resp ~ var, data = d_keep)              # linear
q_mod		<- lm(resp ~ var + I(var^2), data = d_keep)   # quadratic

model_name	<- "Creep - weekly displacement"
...
reglin <- data.frame(
    x = seq(
        extendrange(d_keep$var, f = .07)[1],
        extendrange(d_keep$var, f = .07)[2],
        .01
    ),
    y = NA
)
...
quartz.save(
    file = file.path(dir_plt, "models", "mdl_21.png"),
    type = "png",
    dpi = 300
)
