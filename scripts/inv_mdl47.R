...
var_1		 	<- df_modelvar $ htemp_w_snowpack
var_1_name 		<- "temperatures"
var_1_unit 		<- "°C"
var_1_breaks	<-  seq(0, 25, 5/2)

var_2			<- df_modelvar $ rain_on_snowpack
var_2[var_2 >= 6] <- NA
var_2_name 		<- "rainfalls"
var_2_unit 		<- "mm/h"
var_2_breaks	<-  seq(0, 5, 1/2)

#	model
model_name		<- "Temperatures and rainfalls effects in presence of a snowpack"
model_formulae	<- response ~ var_1 + var_2
...
curve$x[curve$x <= min(var_2, na.rm = TRUE)]	<- NA
curve$y[curve$x <= min(var_2, na.rm = TRUE)]	<- NA
curve$x[curve$x >= max(var_2, na.rm = TRUE)]	<- NA
curve$y[curve$x >= max(var_2, na.rm = TRUE)]	<- NA
...
quartz.save(
    file = file.path(dir_exp, "mdl_47.png"),
    type = "png",
    dpi  = 300
)
